...
public class ScheduledTasks {
	...
	@Value("${monthly_upload_url}")
	String monthly_upload_url;
	...
	public void performDailyMigration() throws IOException {
		...
		File file;
		try {
			file = ResourceUtils.getFile("file:json/daily_upload.json");
			...
			log.info("Json file not found for daily upload");
		}
	...
	/**
	 * THis API used to schedule the task on monthly basis. 
	 * @throws IOException
	 */
	...
	public void performMonthlyMigration() throws IOException {
		if (ScheduledTasks.checkThirdBusinessDay()) {
			...
			File file;
			try {
				file = ResourceUtils.getFile("file:json/monthly_upload.json");
				String content = new String(Files.readAllBytes(file.toPath()));
				...
				try {
					ResponseEntity<String> response = restTemplate.postForEntity(monthly_upload_url, request, String.class );
					System.out.println(response.getStatusCode());
				} catch (HttpStatusCodeException e) {
					String errorpayload = e.getResponseBodyAsString();
					log.info(errorpayload);
				...
				log.info("Json File not found for monthly_upload");
			}
